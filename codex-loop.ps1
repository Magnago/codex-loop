...
function Convert-ListToMarkdown {
    ...
    $values = @(@($Items) | Where-Object { -not [string]::IsNullOrWhiteSpace([string]$_) } | Select-Object -First $MaxItems)
    if ($values.Count -eq 0) {
        return "- None recorded."
    }

    return (($values | ForEach-Object {
        $summary = Get-ObjectPropertyValue -Object $_ -Name "summary"
        $command = Get-ObjectPropertyValue -Object $_ -Name "command"
        if (-not [string]::IsNullOrWhiteSpace([string]$summary)) {
            "- $([string]$summary)"
        }
        elseif (-not [string]::IsNullOrWhiteSpace([string]$command)) {
            "- $([string]$command)"
        }
        else {
            "- $([string]$_)"
...
function Format-ProjectContextValue {
    param(
    ...
    return (Limit-Text -Text $clean -MaxChars $MaxChars).Trim()
}

function New-ProjectContextText {
    param(
        [string]$Goal,
        [AllowNull()]$PreviousContext,
        [AllowNull()]$CodexResult,
        [AllowNull()]$LoopResult,
        [string]$GitSummary,
        [int]$Iteration
    )

    $attemptSummary = Get-ObjectPropertyValue -Object $CodexResult -Name "attempt_summary"
    ...
    $notes = Get-ObjectPropertyValue -Object $CodexResult -Name "notes_for_next_run"
    ...
    $commandsRun = Get-ObjectPropertyValue -Object $CodexResult -Name "commands_run"
    $nextPlan = Get-ObjectPropertyValue -Object $CodexResult -Name "next_attempt_plan"
    $actualStatus = Get-ObjectPropertyValue -Object $LoopResult -Name "actual_status"
    $decisionSource = Get-ObjectPropertyValue -Object $LoopResult -Name "decision_source"

    $contextText = if ([string]::IsNullOrWhiteSpace([string]$context)) { "No reusable context was reported." } else { Format-ProjectContextValue -Text ([string]$context) -MaxChars 8000 }
    $summaryText = if ([string]::IsNullOrWhiteSpace([string]$attemptSummary)) { "No attempt summary was reported." } else { Format-ProjectContextValue -Text ([string]$attemptSummary) -MaxChars 1000 }
    $failureText = if ([string]::IsNullOrWhiteSpace([string]$failure)) { "No failure details recorded." } else { Format-ProjectContextValue -Text ([string]$failure) -MaxChars 1500 }
    $notesText = if ([string]::IsNullOrWhiteSpace([string]$notes)) { "No extra notes recorded." } else { Format-ProjectContextValue -Text ([string]$notes) -MaxChars 3000 }
    $gitSummaryText = Format-ProjectContextValue -Text $GitSummary -MaxChars 4000

    return @"
# codex-loop Project Context

This file is generated by codex-loop and passed into later attempts so Codex does not need to rediscover the whole project.

## Goal
$Goal

## Current Reusable Context
$contextText
...
- Summary: $summaryText
- Failure/details: $failureText

## Files Changed Recently
$(Convert-ListToMarkdown -Items $filesChanged)

## Changes Made Recently
$(Convert-ListToMarkdown -Items $changesMade)

## Commands Already Tried
$(Convert-ListToMarkdown -Items $commandsRun -MaxItems 20)

## Suggested Next Plan
$(Convert-ListToMarkdown -Items $nextPlan)

## Notes For Next Run
$notesText

## Git Summary After Latest Attempt
$gitSummaryText
"@
}
...
$projectContextPath = Join-Path $stateRoot "project-context.md"
...
for ($iteration = $startIteration; $iteration -lt ($startIteration + $MaxRuns); $iteration++) {
    ...
    $projectContextText = if (Test-Path -LiteralPath $projectContextPath) {
        Limit-Text -Text (Get-Content -LiteralPath $projectContextPath -Raw) -MaxChars 20000
    }
    else {
        "No reusable project context has been recorded yet. This first attempt may inspect the project broadly; later attempts should use the generated context instead of rediscovering everything."
    }
...
Reusable project context from previous attempts:
$projectContextText

Context reuse rule:
Trust the reusable project context first. Do not reread or rescan the whole project just to rediscover structure that is already recorded there. Only reread files that are directly relevant to the current failure, files you intend to edit, files changed since the recorded context, or files whose details are missing/unclear.
...
Context reuse rule:
This first attempt may inspect the project enough to build a useful map. In the final JSON "context" field, include compact reusable project memory: important files, commands, interfaces, validation facts, and dead ends, so later attempts do not need to rescan the whole project.
...
"@
    ...
    $previousProjectContext = if (Test-Path -LiteralPath $projectContextPath) {
        Get-Content -LiteralPath $projectContextPath -Raw
    }
    else {
        ""
    }
    $updatedProjectContext = New-ProjectContextText -Goal $Goal -PreviousContext $previousProjectContext -CodexResult $codexJsonResult.Object -LoopResult $loopResult -GitSummary $gitSummaryAfter -Iteration $iteration
    Save-Text -Path $projectContextPath -Text ($updatedProjectContext + [Environment]::NewLine)
    Save-Text -Path (Join-Path $runDir "project-context-after.md") -Text ($updatedProjectContext + [Environment]::NewLine)
